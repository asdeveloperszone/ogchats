import { notifyIncomingCall, notifyMissedCall, dismissCallNotification, pushToReceiver } from './notifications.js';
...
// FIX (XSS): escape user-supplied strings before injecting into innerHTML
...
function showPopup(data) {
  ...
  const safeCallerPhoto = escapeHTML(callerPhoto || '');
  const avatarHTML = safeCallerPhoto
    ? `<img src="${safeCallerPhoto}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" />`
    : `<span style="font-size:26px;font-weight:700;color:#fff;">${escapeHTML(callerName.charAt(0).toUpperCase())}</span>`;
    ...
    <div class="gc-popup-inner">
      <div class="gc-left">
        ...
        <div class="gc-info">
          ...
          <div class="gc-name">${safeCallerName}</div>
        </div>
      </div>
  ...
  missedTimer = setTimeout(async () => {
    ...
    // Layer 2: wake the CALLER's closed device so they know B missed their call
    pushToReceiver({
      type:        'missed_call',
      receiverID:  data.callerID,
      senderID:    myID,
      senderName:  callerName,
      senderPhoto: callerPhoto || null,
      callType:    data.callType || 'voice',
      timestamp:   Date.now()
    });
  }, 30000);
...
async function declineCall() {
  ...
  // FIX: resolve callerName/callerPhoto here — they are only defined inside
  // showPopup's scope and are not accessible from declineCall.
  const contacts2    = JSON.parse(localStorage.getItem('aschat_contacts') || '{}');
  const contact2     = contacts2[data.callerID];
  const callerName   = contact2 ? contact2.name  : (data.callerName || 'User');
  const callerPhoto  = contact2 ? contact2.photo : null;
  hidePopup();
  ...
  try {
    ...
    // Layer 2: wake the CALLER's closed device so they know B declined
    pushToReceiver({
      type:        'missed_call',
      receiverID:  data.callerID,
      senderID:    myID,
