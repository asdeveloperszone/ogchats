...
import { pushToReceiver } from './notifications.js';
import {
...
async function fetchIceServers() {
  try {
    ...
    // FIX: import PUSH_SERVER_URL from notifications.js instead of duplicating
    // the hardcoded URL here. If the Railway URL ever changes, only
    // notifications.js needs updating.
    const { PUSH_SERVER_URL: _ICE_BASE } = await import('./notifications.js').catch(() => ({ PUSH_SERVER_URL: 'https://aschatbackend-production.up.railway.app' }));
    const res = await fetch(`${_ICE_BASE}/api/ice-servers`);
...
export async function startCall(type) {
  ...
  try {
    ...
    // Layer 2 push — wake receiver's closed/locked device with an OS call notification
    pushToReceiver({
      type:        'call',
      receiverID:  otherID,
      senderID:    myID,
