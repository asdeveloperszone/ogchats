import { requestNotificationPermission, registerFCMToken } from './notifications.js';
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    if (cachedID && cachedID !== 'null') {
      // FIX: Also request permission for returning users who haven't granted it yet
      // (previously only called registerFCMToken if already granted, so first-time
      //  permission prompt was skipped for users who auto-login via saved session)
      if (Notification.permission === 'granted') {
        registerFCMToken();
      } else if (Notification.permission !== 'denied') {
        // Request permission without blocking the redirect
