import { notifyMessage, notifyPhoto, notifyVoice, clearChatNotifications, unregisterFCMToken, updateSWUnreadState, signalUserActive, registerReengagementSync, registerFCMToken, isViewingChat, requestNotificationPermission } from './notifications.js';
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (Notification.permission === 'granted') {
    registerFCMToken(); // silently refresh/re-register subscription if needed
  } else if (Notification.permission === 'default') {
    // FIX: Show a non-intrusive banner to request permission for users who
    // landed here without going through the login flow (e.g. direct URL, session restore).
...
// FIX: Show a soft notification permission banner for users who never granted permission.
function showNotificationBanner() {
  if (Notification.permission !== "default") return;
  if (document.getElementById("_notifBanner")) return;

  const banner = document.createElement("div");
  banner.id = "_notifBanner";
  banner.innerHTML = `
    <span style="flex:1;">🔔 Enable notifications to receive messages when the app is closed.</span>
    <button id="_notifAllow" style="background:#4F46E5;color:#fff;border:none;border-radius:8px;padding:6px 14px;cursor:pointer;font-weight:600;white-space:nowrap;">Enable</button>
    <button id="_notifDismiss" style="background:transparent;color:#aaa;border:none;cursor:pointer;font-size:18px;line-height:1;padding:0 4px;">&times;</button>
  `;
  banner.style.cssText = [
    "position:fixed", "bottom:72px", "left:12px", "right:12px",
    "background:#1e1e2e", "color:#e2e8f0", "border:1px solid #4F46E5",
    "border-radius:12px", "padding:12px 14px", "display:flex",
    "align-items:center", "gap:10px", "z-index:9998",
    "box-shadow:0 4px 20px rgba(0,0,0,0.4)", "font-size:14px"
  ].join(";");

  document.body.appendChild(banner);

  document.getElementById("_notifAllow").onclick = async () => {
    banner.remove();
    await requestNotificationPermission();
  };
  document.getElementById("_notifDismiss").onclick = () => banner.remove();
...
// FEATURE (WhatsApp-style auto-chat): inbox listener wakes B instantly when A
// sends a first message. chat.js writes inbox/receiverID/senderID on every send.
// inbox/myID is a tiny O(1) node — no full-DB scan needed.
