import { notifyReaction, notifyMessage, notifyPhoto, notifyVoice, clearChatNotifications, signalUserActive, pushToReceiver } from './notifications.js';
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (autoCallParam === 'decline') setTimeout(() => _declineCall(), 1500);
});
...
window.reactToMessage = async function (emoji) {
  ...
  try {
    ...
    const isRemoving = snap.exists() && snap.val() === emoji;
    if (isRemoving) await remove(rRef);
    else {
      await update(ref(db, 'messages/' + chatKey + '/' + activeMessageID + '/reactions'), { [myID]: emoji });
      // Layer 2 push: notify the message owner (otherID) of the reaction
      // Only push when adding a reaction, not when removing
      const msgData = messageDataStore[activeMessageID];
      const msgOwnerID = msgData && msgData.senderID !== myID ? msgData.senderID : otherID;
      pushToReceiver({
        type:        'reaction',
        receiverID:  msgOwnerID,
        senderID:    myID,
        senderName:  localStorage.getItem('aschat_name') || 'Someone',
        senderPhoto: localStorage.getItem('aschat_photo') || null,
        emoji:       emoji,
        timestamp:   Date.now()
      });
    }
...
async function forwardToContact(contact) {
  ...
  try {
    ...
    updateChatListCache(fwdChatKey, { ...payload, id: newRef.key });

    // inbox ping
    try {
      await update(ref(db, 'inbox/' + contact.userID), {
        [myID]: { senderID: myID, timestamp: Date.now() }
      });
    } catch (e) { /* best-effort */ }

    // Layer 2 push — sender notifies receiver's closed device
    pushToReceiver({
      type:        payload.msgType === 'photo' ? 'photo' : payload.msgType === 'audio' ? 'voice' : 'message',
      receiverID:  contact.userID,
      senderID:    myID,
      senderName:  localStorage.getItem('aschat_name') || 'Someone',
      senderPhoto: localStorage.getItem('aschat_photo') || null,
...
window.sendMessage = async function () {
  ...
  try {
    ...
    // inbox ping — lets receiver auto-discover this chat (WhatsApp-style)
    try {
    ...
    // Layer 2 push — sender triggers OS notification on receiver's closed/locked device
    pushToReceiver({
      type:        'message',
      receiverID:  otherID,
      senderID:    myID,
...
async function flushMessageQueue() {
  ...
  for (const item of queue) {
    try {
      ...
      // Layer 2 push for queued messages (sent while offline, now flushed)
      pushToReceiver({
        type:        'message',
