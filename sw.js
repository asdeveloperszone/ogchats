 ...
 * 
 * Fixes for GitHub Pages subdirectory: /ogchats/
 */

const CACHE_VERSION = 'aschat-v9';
...
self.addEventListener('fetch', (event) => {
  ...
  // Never intercept Firebase, Railway backend, or other external API calls
  if (
  ...
  if (data.type === 'call' && action === 'decline') {
    event.waitUntil(
      self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(clients => {
        const openClients = clients.filter(c => c.visibilityState !== undefined);
        if (openClients.length > 0) {
          openClients.forEach(c => c.postMessage({ type: 'DECLINE_CALL_FROM_NOTIFICATION', callerID: data.callerID }));
        } else {
          // App is fully closed — open chat.html with autocall=decline so it auto-declines on load
          const chatURL = buildChatURL(data.callerID, data.callerName) + '&autocall=decline';
